...
(
    ...
    `id`          BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '用户财产ID',
    ...
    `status`      SMALLINT  DEFAULT 300 COMMENT '当前账户状态',
    ...
    `is_deleted`  INT       DEFAULT 0 COMMENT '资产是否已被删除'
...
(
    ...
    `gender`          TINYINT               DEFAULT 2 COMMENT '用户性别',
    ...
    `status`          SMALLINT              DEFAULT 300 COMMENT '用户当前状态',
...
(
    `id`           BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '对话ID', -- 主键，自增长
    ...
    `status`       SMALLINT              DEFAULT 100 COMMENT '实例状态',
...
(
    `id`           BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '公告ID', -- 主键，自增长
    `uid`          BIGINT       NOT NULL COMMENT '发布者ID',           -- 不能为空
    `title`        VARCHAR(128) NOT NULL COMMENT '标题',
    `content`      TEXT         NOT NULL COMMENT '内容',
    `accessary`    TEXT                  DEFAULT NULL COMMENT '附加的其他类型的媒体内容(JSON格式)',
...
(
    `id`           BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '文章ID', -- 主键，自增长
    `uid`          BIGINT       NOT NULL COMMENT '发布者ID',           -- 不能为空
    `title`        VARCHAR(128) NOT NULL COMMENT '标题',
...
(
    ...
    `comment_num`  INT                DEFAULT 0 COMMENT '评论量',
    `star_num`     INT                DEFAULT 0 COMMENT '收藏量',
    `status`       SMALLINT           DEFAULT 100 COMMENT '当前状态',
